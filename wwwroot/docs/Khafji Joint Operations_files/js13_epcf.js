...
EPCM.DSM.registerAll=function (iVuID, key, tstamp, iframeNameDynamicIview, noNavAway){
if(noNavAway){
key += "noNavAway";}
...
|| ((iframeNameDynamicIview && iframeNameDynamicIview.indexOf("ipdn") == 0) && (iframeNameDynamicIview.indexOf("_frame") > 4) && (typeof LSAPI !== "undefined" && LSAPI.AFPPlugin && LSAPI.AFPPlugin.model))){
key += "dynamicExists";}
...
EPCM.DSM3.filterGusids=function (sks){
var aKeys=[];
var finalString="";
...
filteredSKS=EPCM.DSM3.filterGusids(rt.SerKeyString);
cset.SKS.push(filteredSKS);
...
if (sl.AttrKeyString && sl.AttrKeyString.length > 0) cset.AKS.push(sl.AttrKeyString);
...
if(sl.SerKeyString && sl.SerKeyString.length > 0){
if(fromCloseDynamic){
if(sl.SerKeyString.indexOf("dynamicExists") > -1){
...
if((!doNotClearData) && (!fromCloseDynamic)){
sl.clearData();}
if(ms.SerKeyString && ms.SerKeyString.indexOf("dynamicExists") > -1){
ms.SerKeyString=ms.SerKeyString.replace(/dynamicExists/g,"");}
...
var mapKeyReceived, data=this.getParamMapAsBody(cmd, "SAVE", mapKeySend, terminateOnParent, null, true);
...
EPCM.DSM3.getParamMapAsBody=function (cmd, paramCmd, paramKey, terminateOnParent, currentDSM, clearStrings){
if (!currentDSM){
currentDSM=this;}
var pm=[];
